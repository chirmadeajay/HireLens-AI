...
def get_groq_api_key() -> str:
    """
    Safely loads GROQ_API_KEY without showing it in the app UI.
    Priority:
    1. Streamlit secrets on Streamlit Cloud
    2. Local environment variable for local testing
    """
    try:
...
with st.sidebar:
    ...
    st.write("The Groq API key is loaded securely from Streamlit Secrets or local environment variables.")
    api_key = get_groq_api_key()

    if api_key:
        st.success("Groq API key loaded securely.")
    else:
        st.warning("No Groq API key found. The app will still run using keyword fallback mode.")
